/****************************************************************************
 * MagpieResourceScriptLoader.cpp : Implementation of
 * CMagpieResourceScriptLoader.
 * Copyright 2012 Arne Seib.
 * Author: Arne Seib <kontakt@seiberspace.de>
 ****************************************************************************/

#include "StdAfx.h"
#include "MagpieResourceScriptLoader.h"

/*============================================================================
 * class CMagpieResourceScriptLoader
 */

//----------------------------------------------------------------------------
//  static CreateObject
HRESULT CMagpieResourceScriptLoader::CreateObject(
  HMODULE                                   hModule,
  CMagpieResourceScriptLoaderComObject  *&  pRet)
{
  CMagpieResourceScriptLoaderComObject *newObject = pRet = NULL;
  IF_FAILED_RET(CMagpieResourceScriptLoaderComObject
              ::CreateInstance(&newObject));
  newObject->AddRef();

  HRESULT hr = E_FAIL;
  hr = newObject->Init(hModule);
  if(FAILED(hr))
  {
    newObject->Release();
    return hr;
  }

  pRet = newObject;
  return S_OK;
}

//----------------------------------------------------------------------------
//  CTOR
CMagpieResourceScriptLoader::CMagpieResourceScriptLoader() :
  m_hModule(NULL)
{
}

//----------------------------------------------------------------------------
//  FinalConstruct
HRESULT CMagpieResourceScriptLoader::FinalConstruct()
{
	return S_OK;
}

//----------------------------------------------------------------------------
//  FinalRelease
void CMagpieResourceScriptLoader::FinalRelease()
{
  int asd = 0;
}

HRESULT CMagpieResourceScriptLoader::ResolveModuleID(LPCOLESTR lpszModuleID, CString * psRet)
{
  if (!m_hModule)
  {
    return E_UNEXPECTED;
  }

  // Adjust the module id. Forward slashes are not allowed in resource
  // identifiers, so we use '|' instead.
  CString sModuleIDPath(lpszModuleID);
  sModuleIDPath.Replace(_T('/'), _T('|'));

  HRSRC hRes = FindResource(
    m_hModule, sModuleIDPath, MAKEINTRESOURCE(RT_HTML));
  if(hRes)
  {
    if (psRet)
    {
      (*psRet) = sModuleIDPath;
    }
    return S_OK;
  }
  return S_FALSE;
}

HRESULT CMagpieResourceScriptLoader::Init(HMODULE hModule)
{
  m_hModule = hModule;
  return S_OK;
}

//----------------------------------------------------------------------------
//----------------------------------------------------------------------------
// IMagpieScriptLoader implementation

//----------------------------------------------------------------------------
//  HasModuleScript
STDMETHODIMP CMagpieResourceScriptLoader::HasModuleScript(
  const OLECHAR* lpszModuleID)
{
  return ResolveModuleID(lpszModuleID);
}


//----------------------------------------------------------------------------
//  GetModuleScript
STDMETHODIMP CMagpieResourceScriptLoader::GetModuleScript(
  const OLECHAR* lpszModuleID, BSTR * pbsScript)
{
  ENSURE_RETVAL(pbsScript)
  if (!m_hModule)
  {
    return E_UNEXPECTED;
  }

  // Adjust the module id. Forward slashes are not allowed in resource
  // identifiers, so we use '|' instead.
  CString sModuleIDPath(lpszModuleID);
  sModuleIDPath.Replace(_T('/'), _T('|'));

  HRSRC hRes = FindResource(
    m_hModule, sModuleIDPath, MAKEINTRESOURCE(RT_HTML));
  if (hRes)
  {
	  DWORD dwSize = SizeofResource(m_hModule, hRes);
	  HGLOBAL hResLoad = LoadResource(m_hModule, hRes);
	  if (hResLoad)
	  {
		  LPVOID lpResLock = LockResource(hResLoad);
		  if (lpResLock)
		  {
        LPSTR lps = (LPSTR)new BYTE[dwSize + 1];
			  if (!lps)
				  return E_OUTOFMEMORY;
			  memcpy(lps, lpResLock, dwSize);
        lps[dwSize] = 0;
        (*pbsScript) = SysAllocString(CA2WEX<4096>(lps /*, CODEPAGE*/));
        delete [] lps;
        return (*pbsScript) ? S_OK : E_OUTOFMEMORY;
		  }
	  }
  }
  return HRESULT_FROM_WIN32(::GetLastError());
}

//----------------------------------------------------------------------------
//  GetModuleScriptDecorated
STDMETHODIMP CMagpieResourceScriptLoader::GetModuleScriptDecorated(
          const OLECHAR* lpszModuleID,
          const OLECHAR* lpszScriptIntro,
          const OLECHAR* lpszScriptExtro,
          BSTR * pbsScript)
{
  ENSURE_RETVAL(pbsScript)
  if (!m_hModule)
  {
    return E_UNEXPECTED;
  }

  // Adjust the module id. Forward slashes are not allowed in resource
  // identifiers, so we use '|' instead.
  CString sModuleIDPath(lpszModuleID);
  sModuleIDPath.Replace(_T('/'), _T('|'));

  HRSRC hRes = FindResource(
    m_hModule, sModuleIDPath, MAKEINTRESOURCE(RT_HTML));
  if (hRes)
  {
	  DWORD dwSize = SizeofResource(m_hModule, hRes);
	  HGLOBAL hResLoad = LoadResource(m_hModule, hRes);
	  if (hResLoad)
	  {
		  LPVOID lpResLock = LockResource(hResLoad);
		  if (lpResLock)
		  {
        // intro and extro are wide strings, script input is assumed to be UTF8.
        CW2A scriptIntro(lpszScriptIntro);
        CW2A scriptExtro(lpszScriptExtro);
        // all sizes are in bytes
        DWORD introSize = (lpszScriptIntro) ? (DWORD)strlen(scriptIntro) : 0;
        DWORD extroSize = (lpszScriptExtro) ? (DWORD)strlen(scriptExtro) : 0;
        LPSTR buffer = (LPSTR)new BYTE[introSize + dwSize + extroSize + 1];
        if (!buffer) {
				  return E_OUTOFMEMORY;
        }
        memcpy(buffer, scriptIntro, introSize);
			  memcpy(buffer + introSize, lpResLock, dwSize);
        memcpy(buffer + introSize + dwSize, scriptExtro, extroSize);
        buffer[introSize + dwSize + extroSize] = 0;
        (*pbsScript) = SysAllocString(CA2WEX<4096>(buffer /*, CODEPAGE*/));
        delete [] buffer;
        return (*pbsScript) ? S_OK : E_OUTOFMEMORY;
		  }
	  }
  }
  return HRESULT_FROM_WIN32(::GetLastError());
}

/*============================================================================
 * class CMagpieResourceScriptLoader9

//----------------------------------------------------------------------------
//  static CreateObject
HRESULT CMagpieResourceScriptLoader9::CreateObject(
  HMODULE                                   hModule,
  CMagpieResourceScriptLoader9ComObject  *&  pRet)
{
  CMagpieResourceScriptLoader9ComObject *newObject = pRet = NULL;
  IF_FAILED_RET(CMagpieResourceScriptLoader9ComObject
              ::CreateInstance(&newObject));
  newObject->AddRef();

  HRESULT hr = E_FAIL;
  hr = newObject->Init(hModule);
  if(FAILED(hr))
  {
    newObject->Release();
    return hr;
  }

  pRet = newObject;
  return S_OK;
}

//----------------------------------------------------------------------------
//  CTOR
CMagpieResourceScriptLoader9::CMagpieResourceScriptLoader9() :
  m_hModule(NULL)
{
}

//----------------------------------------------------------------------------
//  FinalConstruct
HRESULT CMagpieResourceScriptLoader9::FinalConstruct()
{
	return S_OK;
}

//----------------------------------------------------------------------------
//  FinalRelease
void CMagpieResourceScriptLoader9::FinalRelease()
{
  int asd = 0;
}

HRESULT CMagpieResourceScriptLoader9::ResolveModuleID(LPCOLESTR lpszModuleID, CString * psRet)
{
  if (!m_hModule)
  {
    return E_UNEXPECTED;
  }

  // Adjust the module id. Forward slashes are not allowed in resource
  // identifiers, so we use '|' instead.
  CString sModuleIDPath(lpszModuleID);
  sModuleIDPath.Replace(_T('/'), _T('|'));

  HRSRC hRes = FindResource(
    m_hModule, sModuleIDPath, MAKEINTRESOURCE(RT_HTML));
  if(hRes)
  {
    if (psRet)
    {
      (*psRet) = sModuleIDPath;
    }
    return S_OK;
  }
  return S_FALSE;
}

HRESULT CMagpieResourceScriptLoader9::Init(HMODULE hModule)
{
  m_hModule = hModule;
  return S_OK;
}

//----------------------------------------------------------------------------
//----------------------------------------------------------------------------
// IMagpieScriptLoader implementation

//----------------------------------------------------------------------------
//  HasModuleScript
STDMETHODIMP CMagpieResourceScriptLoader9::HasModuleScript(
  const OLECHAR* lpszModuleID)
{
  return ResolveModuleID(lpszModuleID);
}


//----------------------------------------------------------------------------
//  GetModuleScript
STDMETHODIMP CMagpieResourceScriptLoader9::GetModuleScript(
  const OLECHAR* lpszModuleID, BSTR * pbsScript)
{
  ENSURE_RETVAL(pbsScript)
  if (!m_hModule)
  {
    return E_UNEXPECTED;
  }

  // Adjust the module id. Forward slashes are not allowed in resource
  // identifiers, so we use '|' instead.
  CString sModuleIDPath(lpszModuleID);
  sModuleIDPath.Replace(_T('/'), _T('|'));

  HRSRC hRes = FindResource(
    m_hModule, sModuleIDPath, MAKEINTRESOURCE(RT_HTML));
  if (hRes)
  {
	  DWORD dwSize = SizeofResource(m_hModule, hRes);
	  HGLOBAL hResLoad = LoadResource(m_hModule, hRes);
	  if (hResLoad)
	  {
		  LPVOID lpResLock = LockResource(hResLoad);
		  if (lpResLock)
		  {
        DWORD introSize = (DWORD)strlen(gJscript9ModuleWrapperIntro);
        DWORD extroSize = (DWORD)strlen(gJscript9ModuleWrapperExtro);
        LPSTR buffer = (LPSTR)new BYTE[introSize + dwSize + extroSize + 1];
        if (!buffer) {
				  return E_OUTOFMEMORY;
        }
			  memcpy(buffer, gJscript9ModuleWrapperIntro, introSize);
			  memcpy(buffer + introSize, lpResLock, dwSize);
			  memcpy(buffer + introSize + dwSize, gJscript9ModuleWrapperExtro, extroSize);
        buffer[introSize + dwSize + extroSize] = 0;
        (*pbsScript) = SysAllocString(CA2WEX<4096>(buffer));
        delete [] buffer;
        return (*pbsScript) ? S_OK : E_OUTOFMEMORY;
		  }
	  }
  }
  return HRESULT_FROM_WIN32(::GetLastError());
}

*/